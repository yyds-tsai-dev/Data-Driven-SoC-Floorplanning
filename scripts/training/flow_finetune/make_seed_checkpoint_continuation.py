#!/usr/bin/env python3
"""Turn a flow checkpoint into a resumable latest.pt at step 0 (round-2 continuation).
...
for _p in (REPO / "src" / "solver", REPO / "FloorSet" / "iccad2026contest", REPO / "FloorSet"):
...
SRC = Path(os.environ.get("FT_SEED_SRC", str(REPO / "artifacts/flow_finetune_round1_0828_tailT24_300k/flow_ft0828_ftv1_tailT24_w0-89_lr2e-5_wu1k_bs12_s300k/final.pt")))
...
def main():
    ...
    args = dict(src["args"])
    args["training_method"] = args.get("training_method", "flow_matching_v1")  # already a FLOW_METHODS tag
