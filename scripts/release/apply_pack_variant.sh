...
# Edits src/shipping/op_wrapper.py (budget table string, FLOW_CKPT filename) and
# scripts/pack_cadc1013.sh (which flow checkpoint is copied). Idempotent; prints the resulting lines.
set -e
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"; cd $ROOT
W=src/shipping/op_wrapper.py; P=scripts/pack_cadc1013.sh
MID=$(cat artifacts/newbox_gate_runs_0821/budget_table_mid.txt); K20=""  # k20 tail table was rejected (docs §18b); variant kept for the record
...
PY
if [ "$CK" = "$FT2" ]; then
  [ -f submission/cadc1013/checkpoints/$FT2 ] || cp artifacts/flow_finetune_round2_0829_tailT12_250k/flow_ft0829_tailT12_lr1e-5_250k_ema.pt submission/cadc1013/checkpoints/$FT2
fi
