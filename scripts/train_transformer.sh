...
ENCODER="${ENCODER:-graph-transformer}"
...
CHECKPOINT_PREFIX="${CHECKPOINT_PREFIX:-gnn_transformer}"
...
DEFAULT_TAG="${DATE_STAMP}_ns${NUM_SAMPLES}_ep${EPOCHS}_enc${ENCODER_TAG}_h${HIDDEN_DIM}_l${LAYERS}_acc${ACCUMULATION_STEPS}_heads${NUM_HEADS}"
...
uv run -m floorset_arch.training.train \
  ...
  "${EXTRA_ARGS[@]}" | tee "$LOG_DIR/train_arch_v11_transformer_${LOG_TAG}.log"
