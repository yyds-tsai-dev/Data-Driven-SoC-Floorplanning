...
**Goal:** Add large-case relative-order candidate search, policy-controlled dirty-sample training, and visible `.env` defaults.
...
- `src/floorset_arch/training/train.py`: support clean-sample policy, weighted dirty geometry samples, strict skip mode, and clean/dirty/skipped counts.
...
- [ ] Implement clean-sample helpers and policy-controlled dirty-sample handling in `train.py`.
