...
## 實驗 14(0805):PARTNER_GPU_ARM(commit f485685)A/B — 兩檔位不促轉

設計(agent):phase-A 執行期間 GPU 採第二波 direct 候選 → phase B(RK 加速)真 cost
選擇;anti-白扣守衛(MIN_A=0.35 + 2·est);K∝slice;39 tests + 全套 834 綠。

| 檔位 | pair A | pair B | 判決 |
|---|---|---|---|
| 3.5(steps10+RK)| −0.0048 | −0.0029 | 均值 −0.0039 未過 −0.005 gate,avg +0.06s → **不促轉**(方向真但太小)|
| goal(st2;TS0=0.05 解鎖)| −0.0048 | **+0.0510** | MIN_A25 +0.056;進場時 phase A 被砍破臨界 → **判死** |

副產品(訓練線收官):st10→8→6→4→2 單調改善 = **目標檔 prior-保真度斜率為負**
(更好的 prior 於固定牆鐘更差)→ 2026-08-05 調研報告的訓練 gate(n≥100 帶斜率
非負)於目標操作點不通過,**cost-FT 訓練線對本目標正式關閉**。
