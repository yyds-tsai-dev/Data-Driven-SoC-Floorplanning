...
- Full-100 control/treatment 共用：`PARTNER_DIRECT_MIN=2.5`、`PARTNER_PRESCREEN_V=1`、`PARTNER_NREF=15`、`PARTNER_OVERSAMPLE=4`、`PARTNER_TAG_ANCHOR_EXTRA=3`、`VKILL_OFF=1`
...
每個 policy union candidate 使用 candidate-stable seed、相同 refine allowance，並固定以 `_worker_refine(..., tag_anchor=False)` 執行。這是 source-neutral 的 **standardized unanchored refine**，不重現 production `PARTNER_TAG_ANCHOR_EXTRA=3` 對 top-3 candidates 額外執行 anchored refine 的 portfolio；trace command 雖保留並記錄 full-100 環境變數，該變數不會改變 trace 的固定 unanchored payload。Official offline score 由既有 `gen_decoder_probe.score_case()` 計算；runtime 與 median 同設為 1，因此此處只比較 no-runtime-equivalent geometry/constraint cost。

這不是 full solver replay：它不包含 production anchored duplicates、concurrent column restarts、shared absolute deadline、fusion、phase-B 與 vkill；所有結論只標為 standardized-unanchored candidate-path evidence。

## Raw / prescreen / standardized-unanchored refine 證據
...
- 低/中 N 的 Retrieval unanchored-refine cost 約 2.19–4.08；Direct winners 約 1.03–1.30。
...
- **Retrieval standardized-unanchored refined winner：0/6 cases。**
- maximum quota 相對 forced：2 better / 4 equal / 0 worse。
- 兩個改善完全來自保留額外 Direct：
  - case 79：1.278166 → 1.194552（Δ -0.083614）；
  - case 98：1.101485 → 1.093690（Δ -0.007795）。

Maximum quota 真正選入 Retrieval 的兩案，在 standardized unanchored refine 下 Retrieval 仍遠差於 Direct winner：
...
這代表 shared cheap prescreen 對 standardized unanchored Retrieval 路徑有 false positives：即使 Retrieval 被排到全體候選第 0/1 名，經同一 unanchored refine 後仍明顯較差。此結果不能排除 production anchored duplicate 在個別 case 改變相對次序。

在本次 sampled standardized-unanchored candidate-path trace 中，match metadata 沒有提供可驗證的 selector。Case 99 有本次最佳 observed distance/confidence 組合（distance 0.0687、confidence 0.1165），兩個 Retrieval refine cost 仍為 3.065407 / 2.157823，而 Direct winner 為 1.383217。因 sampled trace 沒有 Retrieval winner，現有資料無法建立或驗證以 `N`、retrieval distance、match cost、confidence、D4 transform 或 shared rank 為基礎的 winner gate；這不是對 production anchored portfolio、所有可能資料或 selector 的普遍不可能性證明。

Raw transferred layouts 的 official cost 均為 infeasible penalty 10；artifact 沒有 raw violation-type breakdown，因此不能把 infeasibility 單獨歸因於 overlap。Standardized unanchored refine 可讓絕大多數 sampled candidates feasible，但沒有產生 Retrieval winner。這個 observed failure mode 與 unanchored transfer/raw-geometry/refine 路徑未保留可用優勢一致，但本實驗沒有重現 production anchored duplicates，也沒有介入 source teacher，不能據此判定 anchored portfolio 或更好的 teacher 在所有 consumer 設計下都無效。
...
1. sampled standardized-unanchored stratified/tail candidate-path traces 的 Retrieval refined winner 為 0；
...
3. sampled stratified/tail candidate-path traces 中，transferred geometry 沒有在 standardized unanchored refine 後形成 Retrieval winner；現有 production full-100 artifact 沒有 source provenance，故也沒有 policy-faithful positive evidence 可滿足此晉級條件。
...
既定 CP-SAT teacher gate 的前提是：既有 training `fp_sol` 經同一 Retrieval consumer 已先證明有用，然後才判斷更好的 teacher 能否增加收益。現在 full-100 forced policy 明顯退化，standardized-unanchored sampled trace 也產生 0 個 Retrieval refined winners；同時沒有 policy-faithful source attribution 或可驗證的 cheap selector 提供正向證據，故未滿足啟動 CP-SAT teacher 實驗的先決條件。這是停止投資的 gate 決定，不是 production anchored Retrieval 或 improved teacher 的不可能性證明。若未來補上 policy-faithful provenance 或重新設計 correspondence/consumer，仍必須先重新通過 R4，才能重開 CP-SAT teacher 路徑。
