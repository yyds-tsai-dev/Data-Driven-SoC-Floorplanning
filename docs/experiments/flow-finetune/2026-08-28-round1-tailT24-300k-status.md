...
* A tail-tilted fine-tune of the shipped Flow v1 prior is training on **GPU 2**,
  detached (`setsid nohup`), PID group parented at `artifacts/flow_finetune_round1_0828_tailT24_300k/run_ft.sh`.
* Log: `/ldaphome/yyds-tsai-dev/Data-Driven-SoC-Floorplanning/artifacts/flow_finetune_round1_0828_tailT24_300k/train.log`
* Checkpoints: `artifacts/flow_finetune_round1_0828_tailT24_300k/flow_ft0828_ftv1_tailT24_w0-89_lr2e-5_wu1k_bs12_s300k/`
...
* Every shadow manifest (`artifacts/shadow_hidden_suites/shadow_hidden_{v1,v3,v5,v6}_share/manifest.json`)
...
* The fine-tune therefore trains on **`worker_0 … worker_89` only**:
  ...
  Enforced in `TrainSplitLite` (`artifacts/flow_finetune_round1_0828_tailT24_300k/ft_launch.py`), and the
...
setsid nohup bash artifacts/flow_finetune_round1_0828_tailT24_300k/run_ft.sh > artifacts/flow_finetune_round1_0828_tailT24_300k/train.log 2>&1 &
```

`artifacts/flow_finetune_round1_0828_tailT24_300k/run_ft.sh` sets
`PYTHONPATH=FloorSet/iccad2026contest:FloorSet:partner`, `CUDA_VISIBLE_DEVICES=2`,
`FT_TAIL_TEMP=24`, `FT_MAX_WORKER=89`,
`FT_N_INDEX=artifacts/flow_finetune_round1_0828_tailT24_300k/file_n_index.json`, and runs:

```
uv run python artifacts/flow_finetune_round1_0828_tailT24_300k/ft_launch.py \
  --data-path $ROOT/FloorSet \
  --checkpoint-dir $ROOT/artifacts/flow_finetune_round1_0828_tailT24_300k/flow_ft0828_ftv1_tailT24_w0-89_lr2e-5_wu1k_bs12_s300k \
...
| path | what |
|---|---|
| `artifacts/flow_finetune_round1_0828_tailT24_300k/ft_launch.py` | patches `direct_diffusion_train.FloorplanDatasetLite` → train-split-only, and `.FileShuffleSampler` → tail-tilted sampler, then calls `flow_matching_train.main()`. Trainer code itself is untouched. |
| `artifacts/flow_finetune_round1_0828_tailT24_300k/make_seed_checkpoint.py` | v1's stripped ckpt has no `optimizer`/`sched`, which `main()` requires on resume. Builds `latest.pt` at step 0 with v1's `model`+`ema` and fresh AdamW/LambdaLR state. |
| `artifacts/flow_finetune_round1_0828_tailT24_300k/build_file_n_index.py` + `file_n_index.json` | {relpath → n} for all 9000 lite files (dim-1 of the input tensor is n; no intra-file padding). n∈[21,120], 65–112 files per n, 0 failures. |
| `artifacts/flow_finetune_round1_0828_tailT24_300k/export_gate_ckpt.py` | strips a `step_*.pt` to the shipped `{model,ema,model_config,args,step}` form for gating. |
| `artifacts/flow_finetune_round1_0828_tailT24_300k/run_ft.sh` | the detached launcher above. |
...
```bash
cd /ldaphome/yyds-tsai-dev/Data-Driven-SoC-Floorplanning
CK=artifacts/flow_finetune_round1_0828_tailT24_300k/flow_ft0828_ftv1_tailT24_w0-89_lr2e-5_wu1k_bs12_s300k
uv run python artifacts/flow_finetune_round1_0828_tailT24_300k/export_gate_ckpt.py $CK/step_00025000.pt
# -> artifacts/flow_finetune_round1_0828_tailT24_300k/flow_ft0828_tailT24_lr2e-5_step25k.pt

G=/ldaphome/yyds-tsai-dev/.claude/jobs/06af0e53/tmp/run_gate5.sh
COMMON="PARTNER_COORD_POLISH= PARTNER_BUDGET_TABLE=$(cat artifacts/newbox_gate_runs_0821/budget_table_mid.txt) \
DIRECT_OFF= DIRECT_CKPT=$PWD/artifacts/icdc_topology_prior_training/checkpoints_s2_20k/best.pt \
...
bash $G ftT24_r1  $COMMON FLOW_CKPT=$PWD/artifacts/flow_finetune_round1_0828_tailT24_300k/flow_ft0828_tailT24_lr2e-5_step25k.pt
...
uv run python /ldaphome/yyds-tsai-dev/.claude/jobs/06af0e53/tmp/analyze_pairs.py \
  artifacts/shadow_gate_runs/ftBase_r1_off.json artifacts/shadow_gate_runs/ftBase_r2_off.json -- \
  artifacts/shadow_gate_runs/ftT24_r1_off.json  artifacts/shadow_gate_runs/ftT24_r2_off.json
```
