...
結果檔:`artifacts/shadow_gate_runs/rt{Base,K20,K25}_{r1,r2,v3r1,v3r2,v5r1,v5r2,v6r1,v6r2}.json`;log `~/.claude/jobs/06af0e53/tmp/chainRT*.log`。
...
- 進度:`artifacts/flow_finetune_round2_0829_tailT12_250k/train.log`;ckpt `artifacts/flow_finetune_round2_0829_tailT12_250k/flow_ft0829_ft300k_tailT12_lr1e-5_wu1k_bs12_s250k/`(每 25k 一個 `step_*.pt`,250k 完成)。
- 吞吐 5–15 it/s(與 evaluator 鏈搶 CPU),ETA 08-29 21:45 – 08-30 04:00 UTC。
- Gate 鏈已排隊:`scratchpad/rtaware/chainFT2.sh`(等 `step_00250000.pt` → `export_ema_only.py` → preflight 檢查 step=250000 → 四套 ×2 反序,base = 現包 300k EMA)。結果 `artifacts/shadow_gate_runs/ft2{Base,T12}_{r1,r2}_{off,v3,v5,v6}.json`。
...
A = 現包 env(ft0828 300k EMA + mid 表)、B = FT2(T12 250k EMA)、C = k20、D = FT2 + k20。結果 `artifacts/shadow_gate_runs/fn{A,B,C,D}_r{1..4}_{off,v3,v5,v6}.json`;鏈尾自動印 `analyze_pairs.py`(B/C/D vs A,每套)。k20 臂另用 `rt_pairs.py --M 1.0/1.45` 看 total。預計 08-30 00:30–02:00 UTC 完成。
