...
- 2026-06-05 opt-in `FLOORSET_ENABLE_V10_SOFT_REPAIR=1` full run on the promoted Graph Transformer 0521 checkpoint improved v10 no-runtime slightly from `2.1194` to `2.1082`, but worsened runtime-aware total from `2.6641` to `2.7881`. Runtime tail moved from avg/p90/max `1.23s`/`2.23s`/`6.80s` to `1.40s`/`2.54s`/`9.47s`; keep it opt-in and pair future runs with Conditional Runtime Budget experiments before considering promotion.
- 2026-06-05 removed hard runtime-tail clamp trial `FLOORSET_ENABLE_RUNTIME_TAIL_CLAMP=1` reduced the soft-repair p90 runtime to `1.89s` and max runtime to `8.11s`, but over-clamped quality: no-runtime regressed to `2.2741` and runtime-aware total to `3.0120`. The deleted hard-cap form remains useful evidence for tail control, but future work should use conditional runtime budgeting rather than resurrecting this flag.
- 2026-06-05 removed broad grouping-adjacency trial `FLOORSET_ENABLE_GROUPING_ADJACENCY_BIAS=1` combined with soft repair and runtime clamp regressed further to `2.4989` no-runtime and `3.3102` total. The deleted decoder-side bias was too broad; use the current narrow ambiguous-pair bias instead of globally blending compact cluster keys.
...
- Next deterministic optimization should continue through Conditional Runtime Budget: stop extra passes after the first non-accepted repair trial, cap pair candidates only for slow tiers, and preserve baseline repair budgets for cases where soft violations are falling. Any grouping work should stay on narrow ambiguous-pair bias rather than compact cluster key blending.
...
- 2026-06-11 quality-first HGT retraining defaults use fresh training with `NUM_SAMPLES=800000`, `EPOCHS=6`, `LR=1.5e-4`, `ASPECT_WEIGHT=0.06`, repaired pseudo targets enabled, `HGT_RELATION_GATE_MIN=0.20`, high-risk order/pairwise multipliers `1.8/1.6`, high-risk thresholds `100` blocks or `10` constraints, and full evaluator self-check after every epoch. The evaluator self-check runs the production-default checkpoint-isolation path and updates evaluator-best only from evaluator evidence.
