...
- Next deterministic optimization should target large-case boundary/group repair through the shared v10 risk budget: combine `exp(n/12)` score share, constraint density, and net density instead of gating only on `block_count >= 118` or `test_id`-specific logic.
