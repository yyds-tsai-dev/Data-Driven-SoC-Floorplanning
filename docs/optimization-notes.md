...
- Runtime-tail clamp's former hard-cap opt-in flag was removed after it damaged placement quality more than it helped runtime. Future clamp work should be conditional on observed repair acceptance failures or elapsed candidate budget, not just risk tier.
- Decoder-side grouping adjacency's former broad opt-in flag was removed after full validation regressed badly. Keep using the current narrow same-cluster pairwise-axis bonus; any broader grouping change must revalidate full score before promotion.
