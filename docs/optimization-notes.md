...
- 2026-06-05 opt-in `FLOORSET_ENABLE_RUNTIME_TAIL_CLAMP=1` reduced the soft-repair p90 runtime to `1.89s` and max runtime to `8.11s`, but over-clamped quality: no-runtime regressed to `2.2741` and runtime-aware total to `3.0120`. The clamp is useful evidence for tail control but is not a submission candidate in its hard-cap form.
- 2026-06-05 opt-in `FLOORSET_ENABLE_GROUPING_ADJACENCY_BIAS=1` combined with soft repair and runtime clamp regressed further to `2.4989` no-runtime and `3.3102` total. The first decoder-side grouping bias is too broad; keep it opt-in and narrow future versions to ambiguous local order gaps rather than globally blending compact cluster keys.
...
- Runtime-tail clamp is opt-in through `FLOORSET_ENABLE_RUNTIME_TAIL_CLAMP=1`. It lowers expensive repair knobs after risk/profile expansion, but the first hard-cap settings damaged placement quality more than they helped runtime. Future clamp work should be conditional on observed repair acceptance failures or elapsed candidate budget, not just risk tier.
- Decoder-side grouping adjacency bias is opt-in through `FLOORSET_ENABLE_GROUPING_ADJACENCY_BIAS=1`. It can connect compact-profile cluster members in unit tests, but the full validation regressed badly; future versions should use a small same-cluster pairwise-axis bonus without global compact-key blending, and must revalidate full score before promotion.
...
- Next deterministic optimization should replace the hard runtime-tail clamp with a conditional clamp: stop extra passes after the first non-accepted repair trial, cap pair candidates only for slow tiers, and preserve baseline repair budgets for cases where soft violations are falling. Revisit decoder-side grouping adjacency only after that, using narrow ambiguous-pair bias instead of compact cluster key blending.
