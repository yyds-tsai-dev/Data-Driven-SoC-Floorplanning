...
def _sanitize(case: Mapping[str, Any]) -> dict[str, Any]:
    ...
    n = case["n"]
    if not isinstance(n, int) or n < 0: raise ValueError("n")
...
def fingerprint_case(case: Mapping[str, Any]) -> str:
    return hashlib.sha256(_canon(_sanitize(case)).encode()).hexdigest()

def split_for_id(instance_id: str, heldout_mod: int = 10) -> str:
    if heldout_mod <= 0: raise ValueError("heldout_mod")
    bucket = int(hashlib.sha256(instance_id.encode()).hexdigest()[:16], 16) % heldout_mod
    return "heldout" if bucket == 0 else "train"

def save_sanitized_corpus(path: str | Path, cases: Sequence[Mapping[str, Any]], *, source_root: str | Path | None = None) -> None:
    if source_root is not None and Path(source_root).resolve() != (Path("FloorSet/floorset_lite").resolve()):
