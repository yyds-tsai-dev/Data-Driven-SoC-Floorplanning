...
against the ``flow_matching_v*`` teachers produced by ``flow_train_claude``.
...
payload and shared-GPU etiquette are the ones ``direct_train_claude`` /
``flow_train_claude`` already use, so a distilled checkpoint loads through the
unchanged ``checkpoint_method`` -> ``DirectDenoiser`` -> ``sample_flow`` path
(``my_opt_claude._load_flow_model`` and ``scripts/probes/flow_candidate_probe``
...
def ema_model(shadow, cfg, device):
    ...
    from direct_model_claude import DirectDenoiser
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    # -- lifecycle (mirrors direct_train_claude) ------------------------------
...
def main(argv=None):
    ...
    from direct_model_claude import DirectDenoiser, EMA, known_z_channels
    from diffusion_train import known_target_positions_from_fp
