...
run() {  # $1 tag, $2 ckpt, rest = extra env
  ...
  export FLOW_CKPT=$ck; ( cd FloorSet/iccad2026contest && uv run python $R/scripts/iccad2026_evaluate.py --data-path $R/FloorSet --evaluate $R/partner/contest_optimizer.py --output $T/${tag}.json ) > $T/${tag}.out 2> $T/${tag}.err
