#!/bin/bash
# Chain FD: separate-knob check in ONE chain vs the package env: old / polish-headroom only / ANTITHETIC=0 only, x2 each, mirrored order.
cd /ldaphome/yyds-tsai-dev/Data-Driven-SoC-Floorplanning
T=/ldaphome/yyds-tsai-dev/.claude/jobs/06af0e53/tmp
until grep -q CHAINRT2_DONE $T/chainRT2.log 2>/dev/null; do sleep 30; done
...
POL=(PARTNER_COORD_POLISH=1 PARTNER_COORD_POLISH_HEADROOM_S=0.6 PARTNER_FLOW_ANTITHETIC=1)
ANT=(PARTNER_COORD_POLISH= PARTNER_FLOW_ANTITHETIC=0)
bash $G fdOld_r1 "${CK[@]}" "${OLD[@]}"
bash $G fdPol_r1 "${CK[@]}" "${POL[@]}"
bash $G fdAnt_r1 "${CK[@]}" "${ANT[@]}"
bash $G fdAnt_r2 "${CK[@]}" "${ANT[@]}"
bash $G fdPol_r2 "${CK[@]}" "${POL[@]}"
bash $G fdOld_r2 "${CK[@]}" "${OLD[@]}"
echo CHAINFD_DONE
