#!/bin/bash
# refine stall-stop A/B: control vs +PARTNER_REFINE_STALL_STOP vs +both
# stall stops, stacked on the current dpmpp10 baseline env (same control env
# as scratchpad/cherry1_ab_chain.sh).  3 contemporaneous rounds; only
# same-round deltas are trusted (GPU shared with flow v3 training).
...
run_one () {   # $1 = cfg name, $2 = output suffix
  unset PARTNER_REFINE_STALL_STOP PARTNER_SA_STALL_STOP
  case "$1" in
    on)   export PARTNER_REFINE_STALL_STOP=1 ;;
    both) export PARTNER_REFINE_STALL_STOP=1 PARTNER_SA_STALL_STOP=1 ;;
  esac
  OUT="$ROOT/artifacts/partner_eval/refstall_$1$2.json"
