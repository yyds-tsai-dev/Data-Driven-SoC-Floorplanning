...
prod = {int(t["test_id"]): t for t in json.load(open(SC/"eval_prod_r1base.json"))["test_results"]}
...
print("total cheap-A ", round(ev.compute_total_score([r["final"] for r in rows],ns),6),
...
print("total cheap-A tail n>=95", round(ev.compute_total_score(tailv,ns),6),
...
json.dump(rows,open(SC/"gr_cheap.json","w"),indent=1)
