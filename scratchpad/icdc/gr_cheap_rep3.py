...
prod = {int(t["test_id"]): t for t in json.load(open(SC/"eval_prod_rep3.json"))["test_results"]}
...
print("total cheap-A-r3 ", round(ev.compute_total_score([r["final"] for r in rows],ns),6),
...
print("total cheap-A-r3 tail n>=95", round(ev.compute_total_score(tailv,ns),6),
...
json.dump(rows,open(SC/"gr_cheap_rep3.json","w"),indent=1)
