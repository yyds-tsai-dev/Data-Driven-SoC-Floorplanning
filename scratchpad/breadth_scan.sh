#!/bin/bash
# Restart-breadth scan: configs list now extends procedurally to PARTNER_POOL.
# Low budgets cap depth at the deadline, so breadth may invert the 0723
# POOL46 wash verdict.
#   br_ctrl  : goal tier (tau12+kernel+gate0), POOL default 24 (drift bracket)
#   br_p46   : goal tier, POOL=46 (46 restarts on tail cases)
#   br_p46hi : 3.5s tier, POOL=46 (24+15 was the 0723 test; now 31+15)
...
run_arm br_ctrl  PARTNER_POOL_GATE=0 PARTNER_BUDGET_SCALE=6.0e-5 PARTNER_BUDGET_TAU=12 PARTNER_BUDGET_MIN=0.05 PARTNER_BUDGET_MAX=1.0
run_arm br_p46   PARTNER_POOL=46 PARTNER_POOL_GATE=0 PARTNER_BUDGET_SCALE=6.0e-5 PARTNER_BUDGET_TAU=12 PARTNER_BUDGET_MIN=0.05 PARTNER_BUDGET_MAX=1.0
run_arm br_p46hi PARTNER_POOL=46 PARTNER_BUDGET_MAX=3.5
